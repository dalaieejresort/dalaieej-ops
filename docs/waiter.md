...
- New waiter orders default to Карт so the compact card flow leaves more of the basket visible. Staff can choose another method, then confirm receipt of the displayed amount. There is no separate terminal/bank verification button. This records the staff member's confirmation, not independent bank verification. After saving, clearing, or restoring an ordinary order, the payment method returns to Карт; editing a charge retains its charge method.
